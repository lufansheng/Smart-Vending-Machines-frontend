        ...
        <el-table v-loading="loading" :data="empList" @selection-change="handleSelectionChange">
            ...
            <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
                <template #default="scope">
                    <el-button link type="primary" @click="handleUpdate(scope.row)" v-hasPermi="['manage:emp:edit']">
                        修改
                    </el-button>
                    <el-button link type="primary" @click="handleDelete(scope.row)" v-hasPermi="['manage:emp:remove']">
                        删除
                    </el-button>
                </template>
            </el-table-column>
...
function handleUpdate(row) {
    ...
    getEmp(_id).then(response => {
        ...
        // form.value.status = form.value.status.split(",");
        open.value = true;
        title.value = "修改人员列表";
    });
}
...
function submitForm() {
    proxy.$refs["empRef"].validate(valid => {
        if (valid) {
            // form.value.status = form.value.status.join(",");
